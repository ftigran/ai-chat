...
const CHANNEL_STYLE: Record<string, { label: string; className: string }> = {
  ...
  email: {
    label: "Email",
    className: "bg-violet-900/40 text-violet-400 border border-violet-700/50",
  },
};
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
            ...
            {metrics.totalTickets === 0 ? (
              <p className="text-xs text-gray-500">
                Нет данных. Включите маршрутизацию в чате и отправьте сообщения.
              </p>
            ) : (
              ...
              <div className="divide-y divide-gray-800">
                {[...tickets].reverse().map((ticket) => {
                  ...
                  return (
                    <div
                      key={ticket.id}
                      className="px-4 py-3 flex items-center gap-4 hover:bg-gray-800/50 transition-colors"
                    >
                      <div className="text-xs text-gray-500 w-16 flex-shrink-0">
                        {new Date(ticket.timestamp).toLocaleTimeString("ru-RU", {
                          hour: "2-digit",
                          minute: "2-digit",
                        })}
                      </div>
                      ...
                      <span
                        className={`inline-flex items-center text-xs px-2 py-0.5 rounded-full flex-shrink-0 ${channel.className}`}
                      >
                        {channel.label}
                      </span>
                      <span
                        className={`inline-flex items-center text-xs px-2 py-0.5 rounded-full border flex-shrink-0 ${colors.text} ${colors.bg} ${colors.border}`}
                      >
                        {ticket.agentName}
                      </span>
